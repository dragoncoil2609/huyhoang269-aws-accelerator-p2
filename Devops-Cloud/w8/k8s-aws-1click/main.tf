...
resource "null_resource" "setup_k8s" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "      containers:",
      "      - name: db",
      "        image: mysql:8.0", # Có thể thay bằng postgres:15
      "        env:",
      "        - name: MYSQL_ROOT_PASSWORD",
      "          value: \"root123\"", # Mật khẩu khởi tạo
      "        - name: MYSQL_DATABASE",
      "          value: \"mylabdb\"", # Tên DB khởi tạo
      "        ports:",
      "        - containerPort: 3306",
      "---",
      # Service ClusterIP để bảo vệ DB khỏi Internet, chỉ nội bộ mới gọi được
      "apiVersion: v1",
      "kind: Service",
      "metadata:",
      "  name: database-svc",
      ...
      "        env:",
      "        - name: DB_HOST",
      "          value: \"database-svc\"", # Báo cho Backend biết tên miền của Database
      "        ports:",
